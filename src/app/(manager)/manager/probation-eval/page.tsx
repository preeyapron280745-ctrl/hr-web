...
export default function ManagerProbationEvalPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSuccessMsg("");
    if (!selectedForm) {
      setErrorMsg("กรุณาเลือกพนักงานที่ต้องการประเมิน");
      return;
    }
    if (!result) {
    ...
    <div className="space-y-6">
      <div>
        <h1 className="flex items-center gap-2 text-2xl font-bold text-gray-900">
          ...
          ใบประเมินทดลองงาน (หัวหน้าแผนก)
        </h1>
        <p className="mt-1 text-sm text-gray-600">
          ประเมินผลการทดลองงานของพนักงานในแผนกของคุณ
        </p>
      </div>
          ...
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-gray-600">ยังไม่มีใบประเมินทดลองงาน</p>
                <button
                  onClick={() => setTab("new")}
